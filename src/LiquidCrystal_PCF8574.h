...
/// add by NET234
/// 07/03/2020   optimise use of I2C & adjust init sequence (not full ok)
/// 02/07/2020   optimise clear to full reinit LCD (allow cuting power without re initialise )
///              add special char CLEAR HOME CLREOL and handle /r /n in print  so println works

# define LCD_CLEAR   "\x01"  // do a fulll reinit of lcd
# define LCD_HOME    "\x02"  // fast home
# define LCD_CLREOL  "\x03"  // clear to end of line :)  usefull
